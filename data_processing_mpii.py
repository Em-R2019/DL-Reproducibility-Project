...
def ImageProcessing_Person(im_root, anno_path, screen_path, sample_list, im_outpath, label_outpath, person):
    ...
    for count, info in enumerate(im_list):
        ...
        # Crop images
        face_img = im[facebox.begin[1]:facebox.height + facebox.begin[1],
                   facebox.begin[0]:facebox.begin[0] + facebox.width]
        face_img = cv2.resize(face_img, (224, 224))

        left_img = im[leftbox.begin[1]:leftbox.height + leftbox.begin[1],
                   leftbox.begin[0]:leftbox.width + leftbox.begin[0]]
        left_img = cv2.resize(left_img, (60, 36))

        right_img = im[rightbox.begin[1]:rightbox.height + rightbox.begin[1],
                    rightbox.begin[0]:rightbox.width + rightbox.begin[0]]
        ...
        save_face = ",".join(list(map(str, [facebox.begin[0], facebox.begin[1], facebox.begin[0] + facebox.width,
                                            facebox.begin[1] + facebox.height])))
        save_left = ",".join(list(map(str, [leftbox.begin[0], leftbox.begin[1], leftbox.begin[0] + leftbox.width,
                                            leftbox.begin[1] + leftbox.height])))
        save_right = ",".join(list(map(str, [rightbox.begin[0], rightbox.begin[1], rightbox.begin[0] + rightbox.width,
                                             rightbox.begin[1] + rightbox.height])))

        save_str = " ".join(
            [save_name_face, save_name_left, save_name_right, save_name_grid, save_origin, save_flag, save_gaze,
             save_headrot, save_headtrans, save_ratio, save_face, save_left, save_right])
...
def GetFaceBox(out):
    ...
    points = [out.left_left_corner, out.left_right_corner, out.right_left_corner, out.right_right_corner,
              out.bottom_left_corner, out.bottom_right_corner]
...
def GetGrid(image, face):
    ...
    img[face.begin[0]:face.begin[0] + face.width, face.begin[1]:face.begin[1] + face.height] = np.ones(
